var Slider = (function() {
  ...
  function init(slider, options) {
    ...
    slider.$nav = createNav(slider.slidesCount);
    slider.$el.appendChild(slider.$nav);
  ...
  function loadEvents(slider) {
    slider.$nav.addEventListener('click', function pagesClick(event) {
      slider.page(slider.$pages.indexOf(event.target));
    });

    var arrowsClick = function(event) {
      if (slider.options.autoplay) {
        clearTimeout(slider.slideshow);
        slider.options.autoplay = false;
      }

      // if previous button clicked, subtract one from current slide
      var inc = event.target === slider.$navPrev ? -1 : 1;
      var page = (slider.current + inc + slider.slidesCount) % slider.slidesCount;
      navigate(slider, page, 'next');
    };

    slider.$navNext.addEventListener('click', arrowsClick);
    slider.$navPrev.addEventListener('click', arrowsClick);
